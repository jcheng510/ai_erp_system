...
async function executeAnalyzeData(params: any, ctx: AIAgentContext): Promise<any> {
  ...
  switch (dataType) {
    ...
    case "inventory": {
      ...
      // Note: inventory table doesn't have unitCost - would need to join with products for costPrice
      const totalValue = allInventory.reduce((sum, i) => {
        return sum + parseFloat(i.quantity?.toString() || "0");
      }, 0);
    ...
    case "orders": {
      ...
      const completedOrders = allOrders.filter(o => o.status === "shipped" || o.status === "delivered");
    ...
    case "procurement": {
      ...
      const pendingPOs = allPOs.filter(po => po.status === "draft" || po.status === "sent");
  ...
  switch (action) {
    ...
    case "create": {
      ...
      const newVendor = await db.insert(vendors).values({
        ...
        type: (data.category || "supplier") as "supplier" | "contractor" | "service",
...
async function executeManageCopacker(params: any, ctx: AIAgentContext): Promise<any> {
  ...
  switch (action) {
    case "list": {
      // Copackers are vendors with type = 'contractor' (co-packers/manufacturers)
      const allVendors = await db.select().from(vendors);
...
export async function getSystemOverview(ctx: AIAgentContext): Promise<any> {
  ...
  const pendingPOs = poStats.filter(po => po.status === "draft" || po.status === "sent").length;
